...
from tkinter import messagebox, scrolledtext,Label, Entry, Button, Text, Scrollbar, messagebox, filedialog
...
def connect_to_computer(computer, user, username, password):
    ...
    result =subprocess.run(net_use_command, shell=True, check=True)
...
def disconnect_from_computer(computer, user):
    ...
    result =subprocess.run(net_delete_command, shell=True, check=True)
    if result.returncode == 0:
...
def log(message):
    """Terminal ekranına mesaj yazan fonksiyon."""
...
# GUI oluşturma
root = tk.Tk()
root.title("Boylam Sınıfı Bilgisayar Temizleme Aracı")
